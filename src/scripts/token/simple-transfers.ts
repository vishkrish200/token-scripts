...
async function main() {
  ...
  if (walletFiles.length === 0) {
    console.error('No destination wallets found');
    process.exit(1);
  }
  
  console.log(`Found ${walletFiles.length} destination wallet files.`);
  console.log(`Mint address: ${mintAddress.toString()}`);
  console.log(`Decimals: ${decimals}`);
  console.log(`Number of transfers: ${numTransfers}`);
  console.log(`Amount per transfer: ${amountPerTransfer} tokens (${amountPerTransfer * 10**decimals} raw)`);
  console.log();
  ...
  console.log(`Main token account: ${mainTokenAccount.address.toString()}`);
  ...
  try {
    ...
    console.log(`Initial balance: ${parseInt(mainAccountInfo.amount.toString()) / 10**decimals} tokens`);
  } catch (error) {
    console.log('Could not get main account balance:', error);
  }
  ...
  for (let i = 0; i < Math.min(numTransfers, walletFiles.length); i++) {
    ...
    console.log(`Transfer pair ${i + 1}/${Math.min(numTransfers, walletFiles.length)}: ${mainKeypair.publicKey.toString()} <-> ${destKeypair.publicKey.toString()}`);
    ...
    try {
      ...
      console.log(`Destination token account: ${destTokenAccount.address.toString()}`);
      
      // Transfer from main to destination
      console.log(`Transferring ${amountPerTransfer} tokens from main to destination...`);
      
      // Calculate the transfer amount and fee
      const transferAmount = BigInt(amountPerTransfer * 10**decimals);
      // Assuming 4% fee (400 basis points)
      const feeBasisPoints = 400;
      ...
      console.log(`Transaction successful: ${signature1}`);
      
      // Add a small delay between transfers
      await new Promise(resolve => setTimeout(resolve, 2000));
      
      // Transfer back from destination to main
      console.log(`Transferring ${amountPerTransfer / 2} tokens from destination to main...`);
      
      // Calculate the transfer amount and fee for the return transfer
      ...
      console.error(`Error in transfer pair ${i + 1}:`, error);
      console.log();
    }
  }
  ...
  try {
    ...
    console.log(`Final main wallet balance: ${parseInt(updatedMainAccountInfo.amount.toString()) / 10**decimals} tokens`);
  } catch (error) {
    console.log('Could not get final main account balance:', error);
  }
