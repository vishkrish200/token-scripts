...
# Step 5: Transfer tokens to the recipient - use private-key-file instead of private-key
echo -e "${BLUE}Transferring tokens to recipient...${NC}"
ts-node src/scripts/token/transfer-checked.ts --env=local --private-key-file="${CREATOR_WALLET}" --mint=${MINT_ADDRESS} --recipient=${RECIPIENT_PUBKEY} --amount=10000

# Step 6: Check token extensions again to see balances
echo -e "${BLUE}Checking token extensions after transfer...${NC}"
ts-node src/scripts/token/check-token-extensions.ts --env=local --mint=${MINT_ADDRESS} --wallet="${CREATOR_WALLET_PATH}"

# Step 7: Transfer tokens back to test more fee collection - use private-key-file instead of private-key
echo -e "${BLUE}Transferring tokens back to creator...${NC}"
ts-node src/scripts/token/transfer-checked.ts --env=local --private-key-file="${RECIPIENT_WALLET}" --mint=${MINT_ADDRESS} --recipient=${CREATOR_PUBKEY} --amount=1000
